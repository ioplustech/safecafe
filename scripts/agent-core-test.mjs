...
assert.equal(storedSessions[0].draft, null)
assert.equal(storedSessions[0].executablePlan, null)
assert.equal(storedSessions[0].draftKey, "")
assert.equal(storedSessions[0].pendingIntentText, "")
assert.equal(storedSessions[0].warningsAccepted, false)
...
const mockFetch = async (_url, init) => {
    ...
    return new Response(
      JSON.stringify({
        choices: [
          {
            message: {
              content: "Review this plan before signing.",
  ...
  if (usePrepareToolCallJson && !body.messages?.some((message) => message.role === "tool")) {
    return new Response(
      JSON.stringify({
        choices: [
          {
  ...
  if (usePrepareToolCallJson) {
    return new Response(
      JSON.stringify({
        choices: [{ message: { content: "I prepared a stake plan for wallet review." } }],
      }),
      { status: 200, headers: { "content-type": "application/json" } },
    )
  }
...
try {
  ...
  assert.equal(prepareStreamText.includes('"type":"tool"'), true)
  assert.equal(prepareStreamText.includes('"name":"prepare_staking_action"'), true)
  assert.equal(prepareStreamText.includes('"kind":"stake"'), true)
  assert.equal(prepareStreamText.includes('"type":"final"'), true)
  usePrepareToolCallJson = false
